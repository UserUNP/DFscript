...
const Player = {
	...
	action: (name: string, items: VariableType[] = [], target: "" | "Default" | "AllPlayers" | "Selection" = ""): Codeblock => {
		if (!(PLAYER_ACTIONS.includes(name))) throw new Error(`Player.action ${name} does not exist`);
		return {
			id: "block",
			block: "player_action",
	...
	if: (condition: string, items: VariableType[] = []): Codeblock => {
		if (!(IFPLAYER_CONDITIONS.includes(condition))) throw new Error(`Player.if ${condition} does not exist`);
		return {
			id: "block",
			block: "if_player",
			args: { "items": items.map((item) => item.compile()) },
